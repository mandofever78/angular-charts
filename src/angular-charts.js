     ...
     */
       ...
       */
        if (config.labels) {
          ...
          .attr('y', function (d) {
              return (height - Math.abs(y(d.y) - y(0)))-5;
          }).style("display", function(d) { 
              return (d.y === 0) ? "none" : "inline"; 
          })
     ...
     */
      function pieChart() {
        var radius = Math.min(width, height) / 2;
          var svg = d3.select(chartContainer[0]).append('svg').attr('width', width + 150).attr('height', height).append('g').attr('transform', 'translate(' + width / 2 + ',' + height / 2 + ')');
        var innerRadius = 0;
        ...
        if (!!config.labels) {
          ...
          }).attr('dy', '.35em').attr('fill', 'white').style('text-anchor', 'middle').text(function (d) {
            return d.data.y[0];
          });
        }
...
//                      "end" : "start";
//              })
//              .text(function(d, i) { return d.data.y[0]; 
//                                   });
//          }
...
angular.module("angularChartsTemplate_left", []).run(["$templateCache", function($templateCache) {
  $templateCache.put("angularChartsTemplate_left",
                     "<div class=\"angular-charts-template\"><div class=\"ac-title\" ng-if=\"acConfig.title !== false\">{{acConfig.title}}</div><div class=\"ac-legend\" ng-show=\"{{acConfig.legend.display}}\"><div ng-repeat=\"l in legends\"><div class=\"ac-legend-box\" ng-attr-style=\"background:{{l.color}};\"></div><div class=\"ac-legend-title\" ng-bind-html=\"l.title\"></div></div></div><div class=\"ac-chart\"></div></div>");
}]);
